...
export default async function handler(req, res) {
  ...
  try {
    const { email, admin_email, otp } = req.body || {};
    ...
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'bbmmwdo.bmm@gmail.com',
        pass: 'kegctljmzbutxupt',
      },
    });

    const mailOptions = {
      from: '"BMM Admin Portal" <bbmmwdo.bmm@gmail.com>',
      to: targetEmail,
    ...
    console.log(`SUCCESS: OTP email sent to ${targetEmail} with code ${otpCode}`);
